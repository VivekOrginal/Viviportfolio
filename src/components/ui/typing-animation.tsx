...
export function TypingAnimation({
  ...
  const isInView = useInView(elementRef as RefObject<Element>, { amount: 0.3, once: false })
  ...
  const animationSourceKey = useMemo(() => (words ? words.join("\0") : (children ?? "")), [words, children])
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={cn("leading-none tracking-[-0.02em]", Component === "span" && "inline-block", className)}
      {...props}
    >
      {displayedText}
      {shouldShowCursor && (
